  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
        <div className="grid grid-cols-[130px_110px_minmax(0,1fr)_130px_130px_220px] gap-3 px-4 py-2.5 bg-gray-50 text-[11px] font-display font-700 text-gray-500 uppercase tracking-widest">
          <div>Return #</div><div>Order</div><div>Customer / Item</div><div>Reason</div><div>Status</div><div>Manage</div>
        </div>
        {loading ? (
          ...
          <div key={row.id} className="grid grid-cols-[130px_110px_minmax(0,1fr)_130px_130px_220px] gap-3 px-4 py-3 border-t border-gray-100 items-center text-sm">
            ...
            <div className="flex flex-wrap gap-1.5">
              <button onClick={() => setSelected(row)} className="btn-secondary text-[11px] py-1.5 px-2">Manage</button>
              {(row.status === "AWAITING_APPROVAL" || row.status === "REQUESTED") && <button onClick={() => updateReturn(row, "APPROVED")} className="text-[11px] bg-green-50 border border-green-200 text-green-700 rounded-md px-2 py-1.5 font-display font-800 flex items-center gap-1"><CheckCircle size={12}/> Approve</button>}
              {(row.status === "AWAITING_APPROVAL" || row.status === "REQUESTED") && <button onClick={() => updateReturn(row, "REJECTED")} className="text-[11px] bg-red-50 border border-red-200 text-red-700 rounded-md px-2 py-1.5 font-display font-800 flex items-center gap-1"><XCircle size={12}/> Reject</button>}
            </div>
          </div>
        ))}
      </div>
