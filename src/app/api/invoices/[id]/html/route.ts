...
export async function GET(_request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const itemRows = doc.lines
    .filter((l: any) => clean(String(l.description ?? "")).trim())
    .map((l: any) => packing
      ? `<tr><td class="qty">${money(l.quantity).toFixed(2).replace(/\.00$/, "")}</td><td class="sku">${esc(l.sku ?? "")}</td><td class="desc packdesc">${esc(clean(l.description)).replace(/\n/g, "<br/>")}</td><td class="packnote">No loose batteries</td></tr>`
