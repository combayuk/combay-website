...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const itemRows = sourceLines
    ...
    .map((l: any) => packing
      ...
      : isCommercial(doc.type)
        ? `<tr><td class="qty">${money(l.quantity).toFixed(2).replace(/\.00$/, "")}</td><td class="desc">${esc(clean(l.description)).replace(/\n/g, "<br/>")}${l.sku ? `<br/><small>SKU: ${esc(l.sku)}</small>` : ""}</td><td class="hs">${esc(hs(l.description))}</td><td class="hs">${esc(origin(l.description) || exportCountry)}</td><td class="num">${fmt(l.unitPrice)}</td><td class="num">${fmt(l.lineTotal)}</td></tr>`
    ...
    .join("") || (packing ? `<tr><td colspan="4" class="center">No line items were found for this packing list.</td></tr>` : isCommercial(doc.type) ? `<tr><td colspan="6" class="center">No line items found.</td></tr>` : `<tr><td colspan="5" class="center">No line items found.</td></tr>`);
