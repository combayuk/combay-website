...
export async function POST(request: Request) {
  ...
  const dbResult = await withDatabase(async () => {
    ...
    const products = productIds.length ? await prisma.product.findMany({ where: { id: { in: productIds } }, include: { category: true } }) : [];
    const productCandidates = products.map((product) => ({ id: product.id, productId: product.id, categorySlug: product.category?.slug ?? "", category: product.category?.name ?? "", brand: product.brand ?? "", manufacturer: product.manufacturer ?? "" }));
    const targetCheck = checkPromotionProductTargets(promotion, productCandidates.length ? productCandidates : productIds);
