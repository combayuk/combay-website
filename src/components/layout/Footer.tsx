    ...
    <footer className="bg-navy-950 text-white/50">
      ...
      <div className="max-w-7xl mx-auto px-4 py-12">
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-8 mb-10">
          ...
          <div className="col-span-2">
            ...
            <p className="text-xs leading-relaxed text-white/30 max-w-xs mb-5">
              {content?.description || "UK-based industrial and commercial equipment specialists. Buy, repair, or sell surplus stock — backed by engineers."}
            </p>
            <div className="space-y-1.5 text-xs text-white/40">
              <a href={`mailto:${contact.salesEmail || "info@combay.co.uk"}`}             className="flex items-center gap-2 hover:text-accent transition-colors">✉ {contact.salesEmail || "info@combay.co.uk"}</a>
              <a href={`mailto:${contact.repairEmail || "service@combay.co.uk"}`}          className="flex items-center gap-2 hover:text-accent transition-colors">✉ {contact.repairEmail || "service@combay.co.uk"} <span className="text-white/20">(repairs)</span></a>
              <a href={`mailto:${contact.procurementEmail || "procurement@combay.co.uk"}`}      className="flex items-center gap-2 hover:text-accent transition-colors">✉ {contact.procurementEmail || "procurement@combay.co.uk"}</a>
              <a href={`tel:${contact.phone || "+447340383334"}`}                    className="flex items-center gap-2 hover:text-accent transition-colors">☎ {contact.phone || "+44 7340 383334"}</a>
              <span className="flex items-center gap-2">⌖ {contact.location || "Chelmsford, Essex, UK"}</span>
            </div>
          </div>
        ...
        <div className="pt-6 border-t border-white/5 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-white/20">
          ...
          <a href={`https://wa.me/${contact.whatsapp || "447340383334"}`} target="_blank" rel="noopener noreferrer"
