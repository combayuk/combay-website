      ...
      <div className="site-shell">
        <div className="mb-9 grid gap-5 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1fr)] lg:items-end">
          <div>
            ...
            <h2 className="section-heading mt-2 text-3xl lg:text-5xl">
              {content.heading} <span className="text-[#C9872F]">{content.accent}</span>
            </h2>
          </div>
        ...
        <div className={`grid gap-4 ${gridClass(cards.length)}`}>
          {cards.map((card, index) => (
            <article key={`${card.title}-${index}`} data-vcms-item="page.blocks" data-vcms-index={index} className="group flex min-h-[330px] flex-col rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition-all duration-200 hover:-translate-y-1 hover:border-[#E8A44A]/55 hover:shadow-xl">
              {card.imageUrl ? (
                <img src={card.imageUrl} alt={card.title} className="mb-5 h-40 w-full rounded-xl object-cover" />
              ) : (
                <div className="mb-5 flex h-16 w-16 items-center justify-center rounded-xl border border-slate-200 bg-slate-50 text-3xl transition-colors group-hover:border-[#E8A44A]/40 group-hover:bg-[#E8A44A]/10">{card.icon || "⚙"}</div>
              )}
              <p className="mb-2 text-xs font-900 uppercase tracking-[0.14em] text-[#C9872F]">{card.subtitle}</p>
              <h3 className="font-display text-xl font-900 tracking-[-0.02em] text-[#2D4F7A]">{card.title}</h3>
