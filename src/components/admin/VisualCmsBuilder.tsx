    ...
    <aside className="flex w-[370px] shrink-0 flex-col border-r border-slate-200 bg-white shadow-xl">
      ...
      <div className="flex-1 overflow-auto p-3">
        ...
        {leftTab === "style" ? <div className="space-y-5">
          ...
          {pageKey === "about" ? <div className="rounded-xl border border-slate-200 bg-[#F8FAFC] p-3"><p className="text-xs font-display font-900">About quote / image section</p><p className="mt-1 text-[10px] text-slate-500">Controls the industrial quote section below Our Story.</p><div className="mt-3 space-y-2"><Field label="Image URL"><input className="input h-9 w-full text-xs" value={content.pages.about.quoteImageUrl || ""} onChange={(e) => mutateContent((draft) => { draft.pages.about.quoteImageUrl = e.target.value; })} /></Field><UploadButton accept="image/png,image/jpeg,image/webp,image/svg+xml" label="Upload/replace image" onUploaded={(url) => mutateContent((draft) => { draft.pages.about.quoteImageUrl = url; })} /><Field label="Name"><input className="input h-9 w-full text-xs" value={content.pages.about.quoteName || ""} onChange={(e) => mutateContent((draft) => { draft.pages.about.quoteName = e.target.value; })} /></Field><Field label="Designation"><input className="input h-9 w-full text-xs" value={content.pages.about.quoteDesignation || ""} onChange={(e) => mutateContent((draft) => { draft.pages.about.quoteDesignation = e.target.value; })} /></Field><Field label="Quote text"><textarea className="input min-h-24 w-full text-xs" value={content.pages.about.quoteText || ""} onChange={(e) => mutateContent((draft) => { draft.pages.about.quoteText = e.target.value; })} /></Field></div></div> : null}
