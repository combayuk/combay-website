...
import type { CmsBlock, CmsCategory, CmsStep, CmsTextStyle, FaqItem, SiteContent, VisualWidget } from "@/lib/siteContent";
...
type LeftTab = "pages" | "widgets" | "categories" | "style";
type PageConfig = { key: PageKey; label: string; path: string; editable: boolean; note?: string };
type CollectionKey = "page.blocks" | "page.steps" | "faq.previewItems" | "home.promotionStrip" | "trust.clients" | "categories" | `faq.groupItems:${string}`;
...
const CATEGORY_ICON_LIBRARY = ["⚙️", "🔌", "🔬", "📟", "🖧", "🎥", "🤖", "📦", "🔩", "🧪", "🛠", "🏭", "💡", "📡", "🔋", "🖥"];
function slugifyCategory(value: string) { return String(value || "category").toLowerCase().replace(/&/g, " and ").replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 64) || "category"; }
...
export default function VisualCmsBuilder() {
  ...
  function applySelectedTextStyleToCanvas(style: CmsTextStyle) {
    if (!selectedTextStyleKey) return;
    const doc = iframeRef.current?.contentDocument;
    const el = doc?.querySelector<HTMLElement>(`[data-vcms-style-key="${selectedTextStyleKey}"]`);
    if (!el) return;
    ...
    if (style.textKind) {
      el.dataset.vcmsTextKind = style.textKind;
      el.classList.toggle("vcms-style-heading", style.textKind === "heading");
      el.classList.toggle("vcms-style-subheading", style.textKind === "subheading");
      el.classList.toggle("vcms-style-caption", style.textKind === "caption");
    }
  }

  function addCategory() {
    if (!content) return;
    const index = content.categories?.length || 0;
    const label = `New category ${index + 1}`;
    ...
    setMessage("Category added in draft. Edit it, then click Save website.");
  }

  function updateCategory(index: number, changes: Partial<CmsCategory>) {
    if (!content?.categories?.[index]) return;
    mutateContent((draft) => {
      draft.categories = [...(draft.categories || [])];
      const current = draft.categories[index];
      if (!current) return;
      const next = { ...current, ...changes };
      if (changes.label && (!changes.slug || changes.slug === current.slug)) next.slug = slugifyCategory(changes.label);
      draft.categories[index] = next as CmsCategory;
    }, false);
    setMessage("Category updated in draft. Click Save website to publish.");
  }
  ...
  function moveCategory(index: number, direction: -1 | 1) {
    mutateContent((draft) => {
      ...
      draft.categories = items.map((category, order) => ({ ...category, order }));
    }, false);
    setMessage("Category order changed in draft. Click Save website.");
  }

  function deleteCategory(index: number) {
    mutateContent((draft) => {
      draft.categories = (draft.categories || []).filter((_, i) => i !== index).map((category, order) => ({ ...category, order }));
    }, false);
    setMessage("Category removed in draft. Click Save website.");
  }
  ...
  function moveVisualWidget(zone: string, index: number, direction: -1 | 1) { if (!zone || !Number.isFinite(index)) return; mutateContent((draft) => { draft.visualWidgets = draft.visualWidgets || {}; const items = [...(draft.visualWidgets[zone] || [])]; const nextIndex = index + direction; if (!items[index] || nextIndex < 0 || nextIndex >= items.length) return; const [item] = items.splice(index, 1); items.splice(nextIndex, 0, item); draft.visualWidgets[zone] = items; }, true); setSelectedWidget((current) => current ? { ...current, index: Math.max(0, current.index + direction) } : current); setMessage("Widget moved and saved."); }
  function updateSelectedWidget(changes: Partial<VisualWidget>) {
    ...
    setMessage("Widget updated and saved. Live preview will refresh.");
  }

  function updateSelectedTextStyle(changes: CmsTextStyle) {
    if (!selectedTextStyleKey) return;
    const current = content?.textStyles?.[selectedTextStyleKey] || {};
    const next = { ...current, ...changes };
    mutateContent((draft) => {
      draft.textStyles = draft.textStyles || {};
      if (!next.fontSize && !next.fontFamily && !next.fontWeight && !next.italic && !next.colour && !next.align && !next.lineHeight && !next.textKind) delete draft.textStyles[selectedTextStyleKey];
      else draft.textStyles[selectedTextStyleKey] = next;
    }, false);
    applySelectedTextStyleToCanvas(next);
    setMessage("Text style updated in preview. Click Save website to publish.");
  }
  ...
  function addCollectionItem(collection: CollectionKey, widget?: WidgetTemplate) {
    ...
    mutateContent((draft) => {
      ...
      else if (collection === "categories") { const nextIndex = draft.categories?.length || 0; draft.categories = [...(draft.categories || []), { label: `New category ${nextIndex + 1}`, slug: slugifyCategory(`New category ${nextIndex + 1}`), description: "New category", image: "/images/categories/real/plc-module.svg", icon: "📦", iconType: "library", order: nextIndex, visible: true } as CmsCategory]; }
  ...
  function injectEditor() {
    ...
    doc.querySelectorAll<HTMLElement>("[data-vcms-collection]").forEach((section) => {
      ...
      if (!section.querySelector(":scope > .vcms-add")) { const add = doc.createElement("button"); add.type = "button"; add.className = "vcms-add"; add.textContent = collection === "categories" ? "+ Add category" : collection.includes("faq") ? "+ Add FAQ" : "+ Add similar item"; add.addEventListener("click", (event) => { event.preventDefault(); event.stopPropagation(); send({ type: "VCMS_ADD_ITEM", collection }); }); section.appendChild(add); }
    ...
    doc.querySelectorAll<HTMLElement>(textSelector).forEach((el) => {
      ...
      if (cmsStyle) {
        ...
        el.style.textAlign = cmsStyle.align || "";
        el.style.lineHeight = cmsStyle.lineHeight || "";
        if (cmsStyle.textKind) {
          el.dataset.vcmsTextKind = cmsStyle.textKind;
          el.classList.toggle("vcms-style-heading", cmsStyle.textKind === "heading");
          el.classList.toggle("vcms-style-subheading", cmsStyle.textKind === "subheading");
          el.classList.toggle("vcms-style-caption", cmsStyle.textKind === "caption");
        }
      }
    ...
    doc.querySelectorAll<HTMLElement>("[data-system-protected], [data-admin-only], .stripe, .checkout, [href*='/checkout'], [href*='/cart']").forEach((el) => { el.dataset.vcmsProtected = "1"; if (!el.querySelector(":scope > .vcms-locked-label")) { const locked = doc.createElement("span"); locked.className = "vcms-locked-label"; locked.textContent = "🚫 locked"; el.appendChild(locked); } });
  }
  ...
  function targetLabel(target: DropTarget) { return target.kind === "zone" ? target.zone : target.collection === "categories" ? "Category cards" : target.collection.includes("faq") ? "FAQ items" : target.collection === "page.steps" ? "Process/steps" : target.collection === "home.promotionStrip" ? "Promotion banner" : "Cards/content"; }
  ...
  if (!content) return <div className="p-4 text-sm text-slate-600">Loading Visual CMS…</div>;
  const pageBlockItems = pageBlocks();
  return <div className="fixed inset-0 z-40 flex overflow-hidden bg-slate-200 text-navy-950">
    <aside className="flex w-[320px] shrink-0 flex-col overflow-hidden border-r border-slate-200 bg-white shadow-xl">
      <div className="border-b border-slate-200 px-3 py-3"><p className="font-mono text-[10px] uppercase tracking-widest text-accent">Combay Visual CMS</p><h1 className="font-display text-lg font-900">Live Website Editor</h1><p className="mt-1 text-[11px] leading-4 text-slate-500">Real widget zones, hover controls and widget inspector. Drag to the yellow line; click a widget to edit.</p></div>
      <div className="grid grid-cols-4 gap-1 border-b border-slate-200 p-1.5">{(["pages", "widgets", "categories", "style"] as LeftTab[]).map((tab) => <button key={tab} type="button" onClick={() => setLeftTab(tab)} className={`rounded-md py-1.5 text-[11px] font-display font-900 capitalize ${leftTab === tab ? "bg-navy-950 text-white" : "bg-slate-50 text-navy-950"}`}>{tab}</button>)}</div>
      <div className="flex-1 overflow-auto p-2.5">
        {!config.editable ? <div className="mb-2 rounded-lg border border-slate-200 bg-slate-50 p-2 text-[11px] leading-4 text-slate-600">
          This page is shown for live-layout accuracy. Checkout/cart/catalogue logic is locked; use the visible locked markers for protected areas.
        </div> : null}
        {leftTab === "pages" ? <div className="space-y-2">{PAGES.map((page) => <button key={page.key} type="button" onClick={() => { setPageKey(page.key); setRefreshKey(Date.now()); setSelectedWidget(null); }} className={`w-full rounded-lg px-3 py-2 text-left text-xs font-display font-800 ${pageKey === page.key ? "bg-navy-950 text-white" : "bg-gray-50 text-navy-900 hover:bg-gray-100"}`}><span>{page.label}</span>{!page.editable ? <span className="ml-2 text-xs">🚫</span> : null}<span className="block text-[10px] font-normal opacity-70">{page.editable ? "Editable page" : page.note || "Visible but protected"}</span></button>)}</div> : null}
        {leftTab === "widgets" ? <div className="space-y-2.5"><p className="text-[11px] leading-4 text-slate-500">Drag a widget onto the live page. Yellow line controls placement. Current target: <strong>{selectedZone || selectedCollection || defaultZone()}</strong>.</p><div className="grid grid-cols-2 gap-2">{WIDGETS.map((widget) => <button key={widget.label} type="button" draggable onDragStart={(event) => { setDraggingWidget(widget); event.dataTransfer.setData("application/x-combay-widget", JSON.stringify(widget)); event.dataTransfer.effectAllowed = "copy"; }} onDragEnd={() => { setDraggingWidget(null); setDropPreview(null); }} onClick={() => addWidget(widget)} className="rounded-lg border border-slate-200 bg-white p-2 text-left hover:border-accent hover:bg-accent/5"><span className="text-lg">{widget.icon}</span><span className="mt-1 block text-[11px] font-display font-900 text-navy-950">{widget.label}</span><span className="mt-0.5 block text-[10px] text-slate-500">Click or drag</span></button>)}</div><div className="rounded-lg border border-accent/30 bg-accent/10 p-2.5 text-[11px] leading-4 text-navy-950">To place image next to text: add a 2-column section, then drag Image into one column and Text/Button into the other. This is saved in CMS JSON and rendered on the live site.</div></div> : null}

        {leftTab === "categories" ? <div className="space-y-3">
          <div className="rounded-lg border border-accent/30 bg-accent/10 p-2.5 text-[11px] leading-4 text-navy-950">
            Manage the category cards shown on the homepage industry strip. Changes are stored in CMS content and update the live website after saving.
          </div>
          <button type="button" onClick={addCategory} className="w-full rounded-lg bg-navy-950 px-3 py-2 text-xs font-display font-900 text-white hover:bg-accent hover:text-navy-950">+ Add new category</button>
          <div className="space-y-3">
            {(content.categories || []).map((category, index) => (
              <div key={`${category.slug}-${index}`} className="rounded-xl border border-slate-200 bg-white p-3">
                <div className="mb-2 flex items-center justify-between gap-2">
                  <div className="flex min-w-0 items-center gap-2">
                    <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-slate-200 bg-slate-50 p-1">
                      {category.iconType === "library" ? <span className="text-xl leading-none">{category.icon || "▣"}</span> : <img src={category.image || category.icon || "/images/categories/real/plc-module.svg"} alt="" className="h-full w-full object-contain" />}
                    </span>
                    <div className="min-w-0">
                      <p className="truncate text-xs font-display font-900 text-navy-950">{category.label}</p>
                      <p className="truncate font-mono text-[10px] text-slate-400">{category.slug}</p>
                    </div>
                  </div>
                  <div className="flex shrink-0 gap-1">
                    <button type="button" onClick={() => moveCategory(index, -1)} className="rounded bg-slate-100 px-2 py-1 text-[10px] font-800">↑</button>
                    <button type="button" onClick={() => moveCategory(index, 1)} className="rounded bg-slate-100 px-2 py-1 text-[10px] font-800">↓</button>
                    <button type="button" onClick={() => deleteCategory(index)} className="rounded bg-red-50 px-2 py-1 text-[10px] font-800 text-red-700">Delete</button>
                  </div>
                </div>
                <div className="space-y-2">
                  <Field label="Category name"><input className="input h-9 w-full text-xs" value={category.label} onChange={(e) => updateCategory(index, { label: e.target.value })} /></Field>
                  <Field label="Slug"><input className="input h-9 w-full font-mono text-xs" value={category.slug} onChange={(e) => updateCategory(index, { slug: slugifyCategory(e.target.value) })} /></Field>
                  <Field label="Description"><input className="input h-9 w-full text-xs" value={category.description || ""} onChange={(e) => updateCategory(index, { description: e.target.value })} /></Field>
                  <div className="grid grid-cols-2 gap-2">
                    <Field label="Icon type"><select className="input h-9 w-full text-xs" value={category.iconType || "image"} onChange={(e) => updateCategory(index, { iconType: e.target.value as CmsCategory["iconType"] })}><option value="image">Image / uploaded icon</option><option value="library">Icon library</option></select></Field>
                    <Field label="Visible"><select className="input h-9 w-full text-xs" value={category.visible === false ? "no" : "yes"} onChange={(e) => updateCategory(index, { visible: e.target.value === "yes" })}><option value="yes">Visible</option><option value="no">Hidden</option></select></Field>
                  </div>
                  {category.iconType === "library" ? <div><p className="mb-1 text-[10px] font-display font-900 uppercase tracking-wide text-slate-500">Choose icon</p><div className="grid grid-cols-8 gap-1">{CATEGORY_ICON_LIBRARY.map((icon) => <button key={`${category.slug}-${icon}`} type="button" onClick={() => updateCategory(index, { icon, iconType: "library" })} className={`flex h-8 items-center justify-center rounded border text-lg ${category.icon === icon ? "border-accent bg-accent/20" : "border-slate-200 bg-white"}`}>{icon}</button>)}</div></div> : <><Field label="Image/icon URL"><input className="input h-9 w-full text-xs" value={category.image || ""} onChange={(e) => updateCategory(index, { image: e.target.value, iconType: "image" })} /></Field><UploadButton accept="image/png,image/jpeg,image/webp,image/svg+xml" label="Upload icon/image" onUploaded={(url) => updateCategory(index, { image: url, icon: url, iconType: "image" })} /></>}
                </div>
              </div>
            ))}
          </div>
        ...
        {leftTab === "style" ? <div className="space-y-3">
          ...
          {selectedWidgetLive ? <WidgetInspector selected={selectedWidgetLive} update={updateSelectedWidget} deleteWidget={() => deleteVisualWidget(selectedWidgetLive.zone, selectedWidgetLive.index)} duplicateWidget={() => duplicateVisualWidget(selectedWidgetLive.zone, selectedWidgetLive.index)} moveWidget={(direction) => moveVisualWidget(selectedWidgetLive.zone, selectedWidgetLive.index, direction)} /> : null}
      ...
      <div className="border-t border-slate-200 p-2.5"><button type="button" onClick={save} disabled={saving} className="w-full rounded-lg bg-accent px-4 py-2.5 text-xs font-display font-900 text-navy-950 hover:bg-accent-dark disabled:opacity-60">{saving ? "Saving…" : "Save website"}</button>{message ? <p className="mt-2 text-xs text-slate-600">{message}</p> : null}</div>
    </aside>
    <main className="flex min-w-0 flex-1 flex-col overflow-hidden"><div className="flex shrink-0 flex-wrap items-center justify-between gap-2 border-b border-slate-300 bg-white px-3 py-2"><div><p className="text-xs text-slate-500">Editing</p><h2 className="font-display text-base font-900 text-navy-950">{config.label} {config.editable ? "" : "— protected"}</h2></div><div className="flex flex-wrap items-center gap-2"><select className="input h-9 min-w-[150px] px-3 text-xs leading-5" value={device} onChange={(e) => setDevice(e.target.value as DeviceMode)}><option value="desktop">PC desktop</option><option value="tablet">Tablet</option><option value="mobile">Mobile</option></select><select className="input h-9 min-w-[150px] px-3 text-xs leading-5" value={String(zoom)} onChange={(e) => setZoom(Number(e.target.value))}><option value="0.72">Fit Width (72%)</option><option value="0.6">60%</option><option value="0.85">85%</option><option value="1">100%</option></select><a href={config.path} target="_blank" rel="noreferrer" className="rounded-md border border-slate-300 bg-white px-2.5 py-2 text-[11px] font-display font-900 text-navy-950 hover:border-accent">Open live page</a></div></div><div className="flex-1 overflow-auto bg-slate-300 p-3"><div className="mx-auto origin-top rounded-xl bg-white shadow-2xl ring-1 ring-slate-400/40" style={{ width: canvasWidth, minHeight: 760, transform: `scale(${zoom})`, transformOrigin: "top center", marginBottom: -(760 * (1 - zoom)) }}><div className="relative h-[760px] overflow-hidden rounded-xl bg-white" onDragOver={(event) => { if (draggingWidget) { event.preventDefault(); event.dataTransfer.dropEffect = "copy"; setDropPreview(dropPreviewAtCanvasPoint(event.clientX, event.clientY)); } }} onDragLeave={() => setDropPreview(null)} onDrop={(event) => { if (!draggingWidget) return; event.preventDefault(); dropWidgetOnCanvas(draggingWidget, event.clientX, event.clientY); }}>
...
function TextStyleInspector({ value, update }: { value: CmsTextStyle; update: (changes: CmsTextStyle) => void }) {
  ...
  return <div className="rounded-xl border border-accent/30 bg-white p-3">
    ...
    <p className="mt-1 text-[10px] text-slate-500">Every visible control below is functional. Changes update the preview immediately and persist after Save website.</p>
    <div className="mt-3 grid grid-cols-2 gap-2">
      <Field label="Text type"><select className={input} value={value.textKind || ""} onChange={(e) => update({ textKind: e.target.value as CmsTextStyle["textKind"] || undefined })}><option value="">Default</option><option value="heading">Heading</option><option value="subheading">Subheading</option><option value="paragraph">Body text</option><option value="caption">Caption</option></select></Field>
      ...
      <Field label="Line height"><select className={input} value={value.lineHeight || ""} onChange={(e) => update({ lineHeight: e.target.value })}><option value="">Default</option><option value="1.15">Tight</option><option value="1.4">Normal</option><option value="1.7">Comfortable</option><option value="2">Loose</option></select></Field>
    ...
    <div className="mt-3 grid grid-cols-3 gap-1">
      {(["left","center","right"] as const).map((align) => <button key={align} type="button" onClick={() => update({ align })} className={`rounded border px-2 py-1.5 text-[11px] font-900 capitalize ${value.align === align ? "border-accent bg-accent/20 text-navy-950" : "border-slate-200 bg-white text-slate-600"}`}>{align}</button>)}
    </div>
    <div className="mt-3 flex flex-wrap gap-2">
      <button type="button" onClick={() => update({ fontWeight: value.fontWeight === "700" ? "" : "700" })} className={`rounded border px-2 py-1 text-xs font-900 ${value.fontWeight === "700" ? "border-accent bg-accent/20" : "border-slate-200 bg-white"}`}>Bold</button>
      <button type="button" onClick={() => update({ italic: !value.italic })} className={`rounded border px-2 py-1 text-xs font-900 italic ${value.italic ? "border-accent bg-accent/20" : "border-slate-200 bg-white"}`}>Italic</button>
    </div>
  </div>;
}

function WidgetInspector({ selected, update, deleteWidget, duplicateWidget, moveWidget }: { selected: SelectedWidget; update: (changes: Partial<VisualWidget>) => void; deleteWidget: () => void; duplicateWidget: () => void; moveWidget: (direction: -1 | 1) => void }) {
  const w = selected.widget;
  const input = "input h-9 w-full text-xs";
  return <div className="rounded-xl border border-accent/30 bg-white p-3"><div className="mb-3 flex items-center justify-between gap-2"><div><p className="text-xs font-display font-900 capitalize">{w.type} widget</p><p className="text-[10px] text-slate-500">{selected.zone}</p></div><div className="flex gap-1"><button type="button" onClick={() => moveWidget(-1)} className="rounded bg-slate-100 px-2 py-1 text-[10px] font-800">↑</button><button type="button" onClick={() => moveWidget(1)} className="rounded bg-slate-100 px-2 py-1 text-[10px] font-800">↓</button><button type="button" onClick={duplicateWidget} className="rounded bg-slate-100 px-2 py-1 text-[10px] font-800">Copy</button><button type="button" onClick={deleteWidget} className="rounded bg-red-50 px-2 py-1 text-[10px] font-800 text-red-700">Delete</button></div></div><div className="space-y-3">
    ...
    {["text","button","card","promotion","section"].includes(w.type) ? <div className="rounded-lg border border-slate-200 bg-slate-50 p-2"><p className="mb-2 text-[10px] font-display font-900 uppercase tracking-wide text-slate-500">Text style</p><div className="grid grid-cols-2 gap-2"><Field label="Font size"><select className={input} value={w.fontSize || ""} onChange={(e) => update({ fontSize: e.target.value })}>{FONT_SIZES.map((size) => <option key={size.label} value={size.value}>{size.label}</option>)}</select></Field><Field label="Font family"><select className={input} value={w.fontFamily || ""} onChange={(e) => update({ fontFamily: e.target.value })}>{FONT_FAMILIES.map((font) => <option key={font.label} value={font.value}>{font.label}</option>)}</select></Field><Field label="Weight"><select className={input} value={w.fontWeight || ""} onChange={(e) => update({ fontWeight: e.target.value })}><option value="">Default</option><option value="400">Regular</option><option value="600">Semi-bold</option><option value="700">Bold</option><option value="900">Heavy</option></select></Field><Field label="Line height"><select className={input} value={w.lineHeight || ""} onChange={(e) => update({ lineHeight: e.target.value })}><option value="">Default</option><option value="1.15">Tight</option><option value="1.4">Normal</option><option value="1.7">Comfortable</option><option value="2">Loose</option></select></Field><Field label="Colour"><input className={input} value={w.textColour || ""} onChange={(e) => update({ textColour: e.target.value })} placeholder="#2D4F7A" /></Field></div><label className="mt-2 flex items-center gap-2 text-xs font-800 text-navy-950"><input type="checkbox" checked={!!w.italic} onChange={(e) => update({ italic: e.target.checked })} /> Italic</label></div> : null}
