...
import type { SiteContent, CmsBlock } from "@/lib/siteContent";
...
export default function VisualCmsBuilder() {
  ...
  const selectedEditableCmsPage = useMemo(() => Boolean(cmsPageKey(pageKey)) || Boolean(policyKey(pageKey)) || pageKey === "faq", [pageKey]);
  function mutateContent(mutator: (draft: SiteContent) => void) { if (!content) return; const draft = cloneContent(content); mutator(draft); setContent(draft); }
  function currentBlocks(c = content): CmsBlock[] { const key = cmsPageKey(pageKey); return key && c ? c.pages[key].blocks : []; }
  function setBlocks(blocks: CmsBlock[]) { const key = cmsPageKey(pageKey); if (!key) return; mutateContent((draft) => { draft.pages[key].blocks = blocks; }); setMessage("Cards changed. Save website to publish."); }
  function addWidget(widget: WidgetTemplate) { if (!content || !cmsPageKey(pageKey)) { setMessage("This page is visible for accuracy but protected from widget editing."); return; } setBlocks([...currentBlocks(), blockFromWidget(widget)]); }
  function duplicateBlock(index: number) { const blocks = currentBlocks(); const source = blocks[index]; if (!source) return; const clone = { ...source, title: `${source.title} copy` }; setBlocks([...blocks.slice(0, index + 1), clone, ...blocks.slice(index + 1)]); }
  function deleteBlock(index: number) { setBlocks(currentBlocks().filter((_, i) => i !== index)); }
  function addSameSizeCard() { const blocks = currentBlocks(); const last = blocks[blocks.length - 1]; const base = last ? { ...last, title: "New card", subtitle: "New card subtitle", body: "Edit this card text directly on the canvas after saving.", imageUrl: "" } : blockFromWidget(WIDGETS[3]); setBlocks([...blocks, base]); }
  function isHomeHidden(section: string) { return Boolean(content?.hiddenSections?.home?.includes(section)); }
  function toggleHomeSection(section: string) { mutateContent((draft) => { draft.hiddenSections = draft.hiddenSections || {}; const current = new Set(draft.hiddenSections.home || []); current.has(section) ? current.delete(section) : current.add(section); draft.hiddenSections.home = Array.from(current); }); setRefreshKey(Date.now()); setMessage("Section visibility changed. Save website to publish."); }
  function togglePageSection(section: string) { const key = cmsPageKey(pageKey); if (!key) return; mutateContent((draft) => { const page = draft.pages[key]; const current = Array.isArray(page.sectionOrder) && page.sectionOrder.length ? [...page.sectionOrder] : PAGE_SECTIONS.map(([s]) => s); page.sectionOrder = current.includes(section) ? current.filter((s) => s !== section) : [...current, section]; }); setRefreshKey(Date.now()); setMessage("Section changed. Save website to publish."); }
  ...
  function injectEditor() {
    ...
    doc.querySelectorAll<HTMLElement>(textSelector).forEach((el) => { if (!el.textContent?.trim() || el.closest("script,style,svg")) return; el.dataset.vcmsText = "1"; const initial = el.textContent.trim(); el.setAttribute("contenteditable", "true"); el.addEventListener("click", (event) => { event.preventDefault(); event.stopPropagation(); window.postMessage({ type: "VCMS_TEXT_SELECTED", text: el.textContent?.trim() || "" }, window.location.origin); }); el.addEventListener("focus", () => window.postMessage({ type: "VCMS_TEXT_SELECTED", text: el.textContent?.trim() || "" }, window.location.origin)); el.addEventListener("blur", () => { const next = el.textContent?.trim() || ""; if (next && next !== initial) window.postMessage({ type: "VCMS_TEXT_UPDATE", oldText: initial, newText: next }, window.location.origin); }); });
  ...
  return <div className="fixed inset-0 z-40 flex bg-slate-200 text-navy-950">
    <aside className="flex w-[350px] shrink-0 flex-col border-r border-slate-200 bg-white shadow-xl">
      ...
      <div className="flex-1 overflow-auto p-3">
        ...
        {leftTab === "widgets" ? <div className="space-y-3"><p className="text-xs text-slate-500">Click a widget to add it as a card with the same width system as existing cards.</p><div className="grid grid-cols-2 gap-2">{WIDGETS.map((widget) => <button key={widget.label} type="button" onClick={() => addWidget(widget)} className="rounded-xl border border-slate-200 bg-white p-3 text-left hover:border-accent hover:bg-accent/5"><span className="text-xl">{widget.icon}</span><span className="mt-2 block text-xs font-display font-900 text-navy-950">{widget.label}</span></button>)}</div></div> : null}
        {leftTab === "style" ? <div className="space-y-5">
          ...
          {cmsPageKey(pageKey) ? <div className="rounded-xl border border-slate-200 p-3"><div className="mb-3 flex items-center justify-between"><p className="text-xs font-display font-900">Cards</p><button type="button" onClick={addSameSizeCard} className="rounded bg-navy-950 px-2 py-1 text-[11px] font-800 text-white">Add same-size card</button></div><div className="space-y-2">{currentBlocks().map((block, index) => <div key={`${block.title}-${index}`} className="rounded bg-slate-50 p-2"><p className="text-xs font-800">{block.title}</p><p className="text-[10px] text-slate-500">Width: {block.width || "quarter"}</p><div className="mt-2 flex gap-2"><button type="button" onClick={() => duplicateBlock(index)} className="rounded bg-white px-2 py-1 text-[11px] font-800 text-navy-950 ring-1 ring-slate-200">Duplicate same size</button><button type="button" onClick={() => deleteBlock(index)} className="rounded bg-red-50 px-2 py-1 text-[11px] font-800 text-red-700">Delete card</button></div></div>)}</div></div> : null}
          ...
          {policyKey(pageKey) ? <div className="rounded-xl border border-slate-200 bg-accent/10 p-3 text-xs leading-5 text-navy-950">This policy page is now CMS-backed. Edit heading, date and paragraphs directly on the page, then save.</div> : null}
        </div> : null}
      </div>
