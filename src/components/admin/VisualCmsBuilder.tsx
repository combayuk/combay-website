...
export default function VisualCmsBuilder() {
  ...
  return <div className="fixed inset-0 z-40 flex overflow-hidden bg-slate-200 text-navy-950">
    <aside className="flex w-[320px] shrink-0 flex-col overflow-hidden border-r border-slate-200 bg-white shadow-xl">
      ...
      <div className="grid grid-cols-3 gap-1 border-b border-slate-200 p-1.5">{(["pages", "widgets", "style"] as LeftTab[]).map((tab) => <button key={tab} type="button" onClick={() => setLeftTab(tab)} className={`rounded-md py-1.5 text-[11px] font-display font-900 capitalize ${leftTab === tab ? "bg-navy-950 text-white" : "bg-slate-50 text-navy-950"}`}>{tab}</button>)}</div>
    ...
    <main className="flex min-w-0 flex-1 flex-col overflow-hidden"><div className="flex shrink-0 flex-wrap items-center justify-between gap-2 border-b border-slate-300 bg-white px-3 py-2"><div><p className="text-xs text-slate-500">Editing</p><h2 className="font-display text-base font-900 text-navy-950">{config.label} {config.editable ? "" : "— protected"}</h2></div><div className="flex flex-wrap items-center gap-2"><select className="input h-8 w-32 text-xs" value={device} onChange={(e) => setDevice(e.target.value as DeviceMode)}><option value="desktop">PC desktop</option><option value="tablet">Tablet</option><option value="mobile">Mobile</option></select><select className="input h-8 w-24 text-xs" value={String(zoom)} onChange={(e) => setZoom(Number(e.target.value))}><option value="0.6">60%</option><option value="0.72">72%</option><option value="0.85">85%</option><option value="1">100%</option></select><a href={config.path} target="_blank" rel="noreferrer" className="rounded-md border border-slate-300 bg-white px-2.5 py-2 text-[11px] font-display font-900 text-navy-950 hover:border-accent">Open live page</a></div></div><div className="flex-1 overflow-auto bg-slate-300 p-3"><div className="mx-auto origin-top rounded-xl bg-white shadow-2xl ring-1 ring-slate-400/40" style={{ width: canvasWidth, minHeight: 760, transform: `scale(${zoom})`, transformOrigin: "top center", marginBottom: -(760 * (1 - zoom)) }}><div className="relative h-[760px] overflow-hidden rounded-xl bg-white" onDragOver={(event) => { if (draggingWidget) { event.preventDefault(); event.dataTransfer.dropEffect = "copy"; setDropPreview(dropPreviewAtCanvasPoint(event.clientX, event.clientY)); } }} onDragLeave={() => setDropPreview(null)} onDrop={(event) => { if (!draggingWidget) return; event.preventDefault(); dropWidgetOnCanvas(draggingWidget, event.clientX, event.clientY); }}>
