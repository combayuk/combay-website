...
type ShopClientProps = { initialQuery?: string; initialCategory?: string };
...
export default function ShopClient({ initialQuery = "", initialCategory = "" }: ShopClientProps) {
  ...
  return (
    <div>
      <div className="bg-navy-950 text-white py-10"><div className="max-w-7xl mx-auto px-4"><p className="font-mono text-xs tracking-widest uppercase text-accent mb-2">Inventory</p><h1 className="font-display font-900 text-3xl lg:text-4xl mb-3">Browse industrial equipment</h1><p className="text-gray-400 text-sm max-w-2xl">Search tested automation, laboratory, test, AV, networking and process equipment by SKU, brand, MPN, model or manufacturer.</p></div></div>
...
function ProductCard({ product }: { product: CatalogProduct }) {
  ...
  return <div className="card card-hover flex flex-col"><div className="bg-gray-50 border-b border-gray-100 aspect-[4/3] flex items-center justify-center relative overflow-hidden">{product.image ? <img src={product.image} alt={product.title} className="object-contain w-full h-full p-4" /> : <div className="text-gray-300 text-4xl">📦</div>}<span className={`absolute top-2 left-2 badge border ${condition.color}`}>{condition.label}</span></div><div className="p-4 flex flex-col flex-1"><p className="font-mono text-[11px] text-gray-400 mb-1">{product.sku} · {product.brand}</p><h3 className="font-display font-700 text-navy-900 text-sm leading-snug mb-3 flex-1">{product.title}</h3><div className="space-y-1 text-xs text-gray-500 mb-3"><p><span className="text-gray-400">MPN:</span> {product.mpn}</p><p><span className="text-gray-400">Category:</span> {product.category}</p></div><div className="flex items-end justify-between mb-3"><div>{product.priceOnRequest || product.price === null ? <span className="font-display font-700 text-sm text-gray-600">Price on request</span> : <span className="font-display font-800 text-lg text-navy-900">£{product.price.toLocaleString("en-GB")}</span>}<p className="text-gray-400 text-xs mt-0.5">Excl. VAT · {product.warranty}</p></div><span className={product.stockQty > 0 ? "text-green-700 text-xs font-600" : "text-red-600 text-xs font-600"}>{stockCopy}</span></div><div className="grid grid-cols-2 gap-2"><Link href={`/shop/${product.slug}`} className="text-center btn-secondary text-xs py-2 px-3">View</Link><Link href={`/shop/${product.slug}?quote=1`} className="text-center btn-primary text-xs py-2 px-3">Quote</Link>{!product.priceOnRequest && product.price !== null && product.stockQty > 0 && <button type="button" onClick={() => addCartItem(product.sku, 1)} className="col-span-2 btn-secondary text-xs py-2 px-3"><ShoppingCart size={13} /> Add to cart</button>}</div></div></div>;
}
