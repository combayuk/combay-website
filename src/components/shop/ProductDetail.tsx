...
import { ZoomIn, FileText, HelpCircle, BookOpen, AlignLeft, X, ShieldCheck, Truck, PackageCheck, ShoppingCart } from "lucide-react";
  ...
  async function sendForm(type: "quote" | "question", form: { name: string; email: string; phone?: string; message: string }) {
    ...
    try {
      const response = await fetch(type === "quote" ? "/api/quotes" : "/api/support", {
        ...
        body: JSON.stringify({ ...form, type, product: { id: product.id, sku: product.sku, title: product.title, slug: product.slug }, source: "product-detail" }),
      });
...
function EnquiryModal({ product, type, loading, formSent, onClose, onSubmit }: { product: CatalogProduct; type: "quote" | "question"; loading: boolean; formSent: { type: "quote" | "question"; reference: string; emailMessage: string } | null; onClose: () => void; onSubmit: (form: { name: string; email: string; phone?: string; message: string }) => void }) { const isQuote = type === "quote"; const [form, setForm] = useState({ name: "", email: "", phone: "", message: "" }); return <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4"><div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 relative"><button onClick={onClose} className="absolute top-4 right-4 text-gray-400 hover:text-gray-700"><X size={20} /></button>{formSent ? <div className="text-center py-8"><div className="text-3xl mb-3">✓</div><h3 className="font-display font-800 text-lg text-navy-950 mb-1">Request received</h3><p className="text-sm text-gray-500 mb-2">Reference: {formSent.reference}</p><p className="text-xs text-gray-400">{formSent.emailMessage}</p></div> : <form onSubmit={(event) => { event.preventDefault(); onSubmit(form); }} className="space-y-4"><div><p className="font-mono text-[10px] text-accent tracking-widest uppercase mb-1">{product.sku}</p><h3 className="font-display font-800 text-xl text-navy-950">{isQuote ? "Request a quote" : "Ask a question"}</h3><p className="text-sm text-gray-500 mt-1">{product.title}</p></div><input required className="input" placeholder="Your name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /><input required type="email" className="input" placeholder="Email address" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} /><input className="input" placeholder="Phone number (optional)" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} /><textarea required className="input min-h-[120px]" value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} placeholder={isQuote ? "Quantity required, delivery location, urgency or any specific requirements..." : "What would you like to know about this item?"} /><button disabled={loading} type="submit" className="btn-primary w-full py-3">{loading ? "Submitting..." : isQuote ? "Submit quote request" : "Submit question"}</button></form>}</div></div>; }
