  ...
  if (!product) {
    return (
      <div className="max-w-3xl mx-auto px-4 py-20 text-center">
        <h1 className="font-display font-800 text-2xl text-navy-950 mb-2">
          Product not found
        </h1>
        <p className="text-gray-500 text-sm mb-5">
          This item may have been sold, archived or imported under a different SKU.
        </p>
...
function ProductDetailView({ product }: { product: CatalogProduct }) {
  ...
  async function sendForm(type: "quote" | "question") {
    setLoading(true);
    const reference = `CB-${type.toUpperCase()}-${Date.now()
      .toString(36)
      .toUpperCase()}`;
    ...
    <div className="bg-white">
      <div className="border-b border-gray-100 py-3">
        <div className="max-w-7xl mx-auto px-4 flex items-center gap-2 text-xs text-gray-400 flex-wrap">
          ...
          <span>/</span>
          <Link href="/shop" className="hover:text-navy-950">
            Shop
          </Link>
          <span>/</span>
          <Link
          ...
          <span>/</span>
          <span className="text-navy-950 font-display font-600 truncate max-w-xs">
            {product.sku}
          </span>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="grid lg:grid-cols-2 gap-10 mb-10">
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2 mb-3">
              <Link
                href={`/shop?category=${product.categorySlug}`}
                className="font-mono text-[10px] text-accent tracking-widest uppercase hover:text-accent-dark"
              >
                {product.category}
              </Link>
            ...
            <h1 className="font-display font-800 text-2xl lg:text-3xl text-navy-950 leading-tight mb-3">
              {product.title}
            </h1>
            ...
            <div className="grid grid-cols-2 gap-x-6 gap-y-1 text-xs mb-5">
              {[
                ...
                <div key={label} className="flex gap-1.5">
                  ...
                  <span className="text-navy-950 font-display font-600">
                    {value}
                  </span>
                </div>
              ))}
            ...
            <div className="bg-surface border border-gray-200 rounded-2xl p-5 mb-5">
              <p className="font-display font-800 text-3xl text-navy-950 mb-1">
                {formattedPrice}
              </p>
              <p className="text-gray-400 text-xs mb-4">
                Excl. VAT · {product.warranty}
              </p>
              ...
              <div className="grid sm:grid-cols-3 gap-2 mb-5">
                ...
                <InfoPill
                  icon={<Truck size={14} />}
                  title="Dispatch"
                  detail={product.stockQty > 0 ? "Ready to ship" : "Confirm first"}
                />
                <InfoPill
              ...
              <div className="text-sm text-gray-600 space-y-2 mb-5">
                <p>
                  <span className="font-display font-700 text-navy-900">
                    Lead time:
                  </span>{" "}
                  {product.leadTime}
                </p>
                ...
                {!product.priceOnRequest &&
                  ...
                  product.stockQty > 0 && (
                    <button
                      ...
                      disabled
                      title="Cart/checkout is Phase 3"
                    >
            ...
            <div className="flex flex-wrap gap-2">
              {[
                ...
                "Serial recorded",
                "UK dispatch",
                "Quote reference issued",
              ].map((text) => (
                <span
                  key={text}
                  className="badge bg-gray-50 text-gray-600 border-gray-200"
                >
                  {text}
                </span>
              ))}
        ...
        <div className="grid lg:grid-cols-[1fr_320px] gap-8">
            ...
            <div className="border-b border-gray-200 flex gap-1 overflow-x-auto mb-6">
              {tabs.map(([value, label, icon]) => (
                <button
                  ...
                  className={`flex items-center gap-1.5 px-4 py-3 text-sm font-display font-600 border-b-2 transition-colors whitespace-nowrap ${
                    tab === value
                      ? "border-accent text-navy-950"
          ...
          <aside className="bg-gray-50 border border-gray-200 rounded-2xl p-5 h-fit">
            <h2 className="font-display font-800 text-lg text-navy-950 mb-3">
              Procurement note
            </h2>
            <p className="text-sm text-gray-600 leading-relaxed mb-4">
              For business buyers, quote requests should carry the product SKU
              and exact MPN so our team can confirm availability, shipping, VAT
              position and any required documentation.
            </p>
            <button
              onClick={() => setModal("quote")}
...
function ProductGallery({
  ...
  return (
    <div>
      <div
        className="bg-surface border border-gray-200 rounded-2xl aspect-square flex items-center justify-center relative overflow-hidden group cursor-zoom-in mb-3"
        onClick={onZoom}
      >
        {product.image ? (
          <img
        ...
        <div className="absolute bottom-3 right-3 bg-white/90 border border-gray-200 rounded-lg p-1.5 opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1.5">
          ...
          <span className="text-[10px] text-gray-500 font-display font-600">
            Zoom
          </span>
        </div>
      </div>
      <div className="flex gap-2">
        {[0, 1, 2].map((item) => (
          <button
            key={item}
            className="w-16 h-16 bg-surface border border-gray-200 rounded-xl flex items-center justify-center hover:border-gray-400 transition-colors"
          >
  ...
  return (
    <div className="border border-gray-200 rounded-xl overflow-hidden">
      {product.specs.map((spec, index) => (
        ...
        >
          <div className="px-4 py-3 font-display font-700 text-navy-950 border-r border-gray-200">
            {spec.label}
          </div>
    ...
    <div className="space-y-3">
      {product.documents.map((document) => (
        <a
          key={document.name}
          href={document.url}
          className="flex items-center justify-between border border-gray-200 rounded-xl p-4 hover:border-accent transition-colors"
        >
          <div className="flex items-center gap-3">
            <FileText size={18} className="text-accent" />
            <div>
              <p className="font-display font-700 text-sm text-navy-950">
                {document.name}
              </p>
              <p className="text-xs text-gray-400">{document.fileType}</p>
            </div>
          </div>
          <span className="text-xs text-accent font-display font-700">
            Open
          </span>
        </a>
      ))}
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 relative">
        ...
        {formSent ? (
          <div className="text-center py-8">
            ...
            <h3 className="font-display font-800 text-lg text-navy-950 mb-1">
              Request received
            </h3>
            <p className="text-sm text-gray-500">
              A reference has been generated and the request has been passed to
              the current Phase 2 API endpoint.
            </p>
          </div>
        ) : (
          <form
          ...
          >
            <div>
              ...
              <h3 className="font-display font-800 text-xl text-navy-950">
                {isQuote ? "Request a quote" : "Ask a question"}
              </h3>
            ...
            <textarea
              ...
              placeholder={
                isQuote
                  ? "Quantity required, delivery location, urgency or any specific requirements..."
