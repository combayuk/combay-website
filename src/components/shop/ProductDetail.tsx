...
function ProductDetailView({ product }: { product: CatalogProduct }) {
  ...
  const canBuy = !product.priceOnRequest && product.price !== null && product.stockQty > 0;
          ...
          <div>
            ...
            <div className="bg-surface border border-gray-200 rounded-2xl p-5 mb-5">
              ...
              <div className="flex flex-col gap-2">
                {canBuy && (
                  <button onClick={addToCart} className="btn-secondary w-full py-3 text-base">
                    <ShoppingCart size={16} /> {added ? "Added to cart" : "Add to cart"}
                  </button>
                )}
                {canBuy && <Link href="/cart" className="btn-primary w-full py-3 text-base">Buy now / view cart →</Link>}
