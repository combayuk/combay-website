...
import { ZoomIn, FileText, HelpCircle, BookOpen, AlignLeft, X, ShieldCheck, Truck, PackageCheck, ShoppingCart, Minus, Plus } from "lucide-react";
...
function ProductGallery({ product, onZoom }: { product: CatalogProduct; onZoom: () => void }) {
  const images = product.images?.length ? product.images : product.image ? [{ url: product.image, alt: product.title, isPrimary: true, sortOrder: 0 }] : [];
  const [activeUrl, setActiveUrl] = useState(images[0]?.url ?? product.image ?? "");
  useEffect(() => setActiveUrl(images[0]?.url ?? product.image ?? ""), [product.id]);
  return <div><div className="bg-surface border border-gray-200 rounded-2xl aspect-square flex items-center justify-center relative overflow-hidden group cursor-zoom-in mb-3" onClick={onZoom}>{activeUrl ? <img src={activeUrl} alt={product.title} className="object-contain w-full h-full p-6" /> : <div className="text-gray-200 text-[7rem] select-none">📦</div>}<div className="absolute bottom-3 right-3 bg-white/90 border border-gray-200 rounded-lg p-1.5 opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1.5"><ZoomIn size={13} className="text-gray-500" /><span className="text-[10px] text-gray-500 font-display font-600">Zoom</span></div></div><div className="flex gap-2 flex-wrap">{images.slice(0, 8).map((image, index) => <button key={`${image.url}-${index}`} onClick={() => setActiveUrl(image.url)} className={`w-16 h-16 bg-surface border rounded-xl flex items-center justify-center hover:border-gray-400 transition-colors ${activeUrl === image.url ? "border-accent" : "border-gray-200"}`}><img src={image.url} alt={image.alt ?? product.title} className="object-contain w-full h-full p-1" /></button>)}</div></div>;
}
