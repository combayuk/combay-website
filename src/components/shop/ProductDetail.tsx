...
import { ZoomIn, ZoomOut, RotateCw, FileText, HelpCircle, BookOpen, AlignLeft, X, ShieldCheck, Truck, PackageCheck, ShoppingCart, Minus, Plus } from "lucide-react";
...
function ProductDetailView({ product }: { product: CatalogProduct }) {
  ...
  return <div className="bg-white"><div className="border-b border-gray-100 py-3"><div className="max-w-7xl mx-auto px-4 flex items-center gap-2 text-xs text-gray-400 flex-wrap"><Link href="/" className="hover:text-navy-950">Home</Link><span>/</span><Link href="/shop" className="hover:text-navy-950">Shop</Link><span>/</span><Link href={`/shop?category=${product.categorySlug}`} className="hover:text-navy-950">{product.category}</Link><span>/</span><span className="text-navy-950 font-display font-600 truncate max-w-xs">{product.sku}</span></div></div><div className="max-w-7xl mx-auto px-4 py-8"><div className="grid lg:grid-cols-2 gap-10 mb-10"><ProductGallery product={product} onZoom={() => setZoom(true)} /><div><div className="flex flex-wrap items-center gap-2 mb-3"><Link href={`/shop?category=${product.categorySlug}`} className="font-mono text-[10px] text-accent tracking-widest uppercase hover:text-accent-dark">{product.category}</Link><span className={`badge border ${condition.color}`}>{condition.label}</span>{availableQty > 0 ? <span className="badge text-green-700 bg-green-50 border-green-200">In stock ({availableQty})</span> : <span className="badge text-red-700 bg-red-50 border-red-200">Out of stock</span>}</div><h1 className="font-display font-800 text-2xl lg:text-3xl text-navy-950 leading-tight mb-3">{product.title}</h1><div className="grid grid-cols-2 gap-x-6 gap-y-1 text-xs mb-5">{[["SKU", product.sku], ["Brand", product.brand], ["Manufacturer", product.manufacturer], ["Model", product.model], ["MPN", product.mpn]].map(([label, value]) => <div key={label} className="flex gap-1.5"><span className="text-gray-400">{label}:</span><span className="text-navy-950 font-display font-600">{value}</span></div>)}</div><div className="bg-surface border border-gray-200 rounded-2xl p-5 mb-5"><p className="font-display font-800 text-3xl text-navy-950 mb-1">{formattedPrice}</p><p className="text-gray-400 text-xs mb-4">Excl. VAT · {product.warranty}</p><div className="grid sm:grid-cols-3 gap-2 mb-5"><InfoPill icon={<ShieldCheck size={14} />} title="Warranty" detail="30-day RTB" /><InfoPill icon={<Truck size={14} />} title="Dispatch" detail={availableQty > 0 ? "Ready to ship" : "Confirm first"} /><InfoPill icon={<PackageCheck size={14} />} title="Condition" detail={condition.label} /></div>{variants.length > 0 && <div className="mb-5"><label className="label">Choose variation</label><select value={selectedVariantId} onChange={(e) => { setSelectedVariantId(e.target.value); setQty(1); }} className="input"><option value="" disabled>Select variation</option>{variants.map((variant) => <option key={variant.id} value={variant.id}>{variant.label}{variant.sku ? ` — ${variant.sku}` : ""} · Stock {variant.stockQty}{variant.price !== null && variant.price !== undefined ? ` · £${Number(variant.price).toFixed(2)}` : ""}</option>)}</select></div>}<div className="mb-5"><label className="label">Quantity</label><div className="inline-flex items-center border border-gray-200 rounded-lg overflow-hidden bg-white"><button type="button" onClick={() => setQty((current) => Math.max(1, current - 1))} className="p-2 hover:bg-gray-50"><Minus size={14} /></button><input type="number" min={1} max={Math.max(1, availableQty)} value={qty} onChange={(e) => setQty(Math.max(1, Math.floor(Number(e.target.value || 1))))} className="w-16 text-center text-sm font-display font-700 outline-none" /><button type="button" onClick={() => setQty((current) => Math.min(Math.max(1, availableQty), current + 1))} className="p-2 hover:bg-gray-50"><Plus size={14} /></button></div>{qty > availableQty && <p className="text-xs text-red-600 mt-1">Only {availableQty} available for the selected option.</p>}</div><div className="text-sm text-gray-600 space-y-2 mb-5"><p><span className="font-display font-700 text-navy-900">Lead time:</span> {product.leadTime}</p><p><span className="font-display font-700 text-navy-900">Condition note:</span> {condition.description}</p></div><div className="flex flex-col gap-2">{canBuy && <button onClick={addToCart} className="btn-secondary w-full py-3 text-base"><ShoppingCart size={16} /> {added ? "Added — view cart" : "Add to Cart"}</button>}<button onClick={() => setModal("quote")} className="btn-primary w-full py-3 text-base">Request a Quote →</button><button onClick={() => setModal("question")} className="btn-secondary w-full py-2.5">Ask a Question</button></div></div><div className="flex flex-wrap gap-2">{["30-day warranty", "Serial recorded", "UK dispatch", "Quote reference issued"].map((text) => <span key={text} className="badge bg-gray-50 text-gray-600 border-gray-200">{text}</span>)}</div></div></div><div className="grid lg:grid-cols-[1fr_320px] gap-8"><div><div className="border-b border-gray-200 flex gap-1 overflow-x-auto mb-6">{tabs.map(([value, label, icon]) => <button key={value} onClick={() => setTab(value)} className={`flex items-center gap-1.5 px-4 py-3 text-sm font-display font-600 border-b-2 transition-colors whitespace-nowrap ${tab === value ? "border-accent text-navy-950" : "border-transparent text-gray-400 hover:text-navy-950"}`}>{icon}{label}</button>)}</div>{tab === "description" && <TextBlock text={product.description} />}{tab === "overview" && <TextBlock text={product.productOverview} />}{tab === "specs" && <SpecsTable product={product} />}{tab === "documents" && <Documents product={product} />}</div><aside className="bg-gray-50 border border-gray-200 rounded-2xl p-5 h-fit"><h2 className="font-display font-800 text-lg text-navy-950 mb-3">Procurement note</h2><p className="text-sm text-gray-600 leading-relaxed mb-4">For business buyers, quote requests should carry the product SKU, selected variation and exact MPN so our team can confirm availability, shipping, VAT position and documentation.</p><button onClick={() => setModal("quote")} className="btn-primary w-full">Request quote for {product.sku}</button></aside></div></div>{zoom && <div className="fixed inset-0 bg-black/85 z-50 flex items-center justify-center p-4" onClick={() => setZoom(false)}><button className="absolute top-4 right-4 text-white/70 hover:text-white"><X size={24} /></button><div className="bg-white rounded-2xl w-full max-w-2xl aspect-square flex items-center justify-center">{product.image ? <img src={product.image} alt={product.title} className="max-h-full max-w-full object-contain" /> : <div className="text-gray-200 text-[10rem]">📦</div>}</div></div>}{modal && <EnquiryModal product={product} type={modal} loading={loading} formSent={formSent} initialQty={qty} selectedVariant={selectedVariant} onClose={() => setModal(null)} onSubmit={(form) => sendForm(modal, form)} />}</div>;
}

function ProductGallery({ product }: { product: CatalogProduct; onZoom?: () => void }) {
  ...
  useEffect(() => { setActiveIndex(0); setSpin(false); setZoomScale(1); }, [product.id]);
  useEffect(() => {
    if (!spin || images.length < 2) return;
    const timer = window.setInterval(() => setActiveIndex((current) => (current + 1) % images.length), 700);
    return () => window.clearInterval(timer);
  }, [spin, images.length]);

  function nextImage(direction: 1 | -1) {
    setSpin(false);
    setActiveIndex((current) => (current + direction + images.length) % images.length);
  }

  return <div>
    <div className="bg-surface border border-gray-200 rounded-2xl aspect-square flex items-center justify-center relative overflow-hidden group cursor-zoom-in mb-3" onClick={() => setZoomOpen(true)}>
      {active?.url ? <img src={active.url} alt={active.alt ?? product.title} className="object-contain w-full h-full p-6" /> : <div className="text-gray-200 text-[7rem] select-none">📦</div>}
      <div className="absolute bottom-3 right-3 bg-white/90 border border-gray-200 rounded-lg p-1.5 opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1.5"><ZoomIn size={13} className="text-gray-500" /><span className="text-[10px] text-gray-500 font-display font-600">Zoom / 360</span></div>
    </div>
    ...
    {zoomOpen && <div className="fixed inset-0 bg-black/90 z-50 flex flex-col" onClick={() => setZoomOpen(false)}>
      <div className="flex items-center justify-between p-4 text-white" onClick={(event) => event.stopPropagation()}>
        <div><p className="font-display font-800 text-sm">Image viewer</p><p className="text-xs text-white/60">{images.length > 1 ? `${activeIndex + 1} of ${images.length}` : "Single image"}</p></div>
        <div className="flex items-center gap-2"><button type="button" onClick={() => setZoomScale((current) => Math.max(1, current - 0.25))} className="border border-white/20 rounded-lg px-3 py-2 text-xs hover:bg-white/10"><ZoomOut size={14} /></button><button type="button" onClick={() => setZoomScale((current) => Math.min(3, current + 0.25))} className="border border-white/20 rounded-lg px-3 py-2 text-xs hover:bg-white/10"><ZoomIn size={14} /></button><button type="button" disabled={images.length < 2} onClick={() => setSpin((current) => !current)} className={`border rounded-lg px-3 py-2 text-xs hover:bg-white/10 flex items-center gap-1 ${spin ? "border-accent text-accent" : "border-white/20"}`}><RotateCw size={14} /> 360 view</button><button type="button" onClick={() => setZoomOpen(false)} className="border border-white/20 rounded-lg px-3 py-2 text-xs hover:bg-white/10"><X size={16} /></button></div>
      </div>
      <div className="flex-1 flex items-center justify-center p-4 relative" onClick={(event) => event.stopPropagation()}>
        {images.length > 1 && <button type="button" onClick={() => nextImage(-1)} className="absolute left-4 top-1/2 -translate-y-1/2 bg-white/10 text-white rounded-full px-4 py-3 hover:bg-white/20">‹</button>}
        {active?.url ? <img src={active.url} alt={active.alt ?? product.title} style={{ transform: `scale(${zoomScale})` }} className="max-h-full max-w-full object-contain transition-transform duration-150" /> : <div className="text-white/20 text-[10rem]">📦</div>}
        {images.length > 1 && <button type="button" onClick={() => nextImage(1)} className="absolute right-4 top-1/2 -translate-y-1/2 bg-white/10 text-white rounded-full px-4 py-3 hover:bg-white/20">›</button>}
      </div>
    </div>}
  </div>;
}
