    ...
    <section className="bg-navy-950 text-white py-16" style={heroStyle}>
      <div className="max-w-7xl mx-auto px-4 grid lg:grid-cols-[1fr_420px] gap-10 items-center">
        <div className="max-w-2xl">
          ...
          <div className="flex flex-wrap gap-3"><Link href={page.primaryHref} className="btn-primary">{page.primaryLabel}</Link><Link href={page.secondaryHref} className="border border-white/30 text-white font-display font-600 px-5 py-2.5 rounded hover:border-white transition-colors">{page.secondaryLabel}</Link></div>
        </div>
...
export default function CmsPageLayout({ page, contact, children, formTitle }: { page: CmsPage; contact?: Contact; children?: React.ReactNode; formTitle?: string }) {
  ...
  const rendered = order.map((section, index) => {
    ...
    if (section === "formOrCta") return children ? <section key={`${section}-${index}`} id="request" className="py-14 bg-white"><div className="max-w-3xl mx-auto px-4"><p className="section-label">Get Started</p><h2 className="section-heading text-3xl mb-2">{formTitle || page.ctaHeading}</h2><Paragraphs text={page.ctaBody} className="text-gray-500 mb-8 text-sm space-y-2" />{children}</div></section> : <section key={`${section}-${index}`} className="py-12 bg-accent"><div className="max-w-3xl mx-auto px-4 text-center"><h2 className="font-display font-900 text-3xl text-navy-900 mb-3">{page.ctaHeading}</h2><Paragraphs text={page.ctaBody} className="text-navy-800 mb-6 text-sm space-y-2" /><div className="flex flex-wrap justify-center gap-3"><Link href={page.ctaPrimaryHref} className="bg-navy-900 text-white font-display font-700 px-6 py-3 rounded hover:bg-navy-800 transition-colors">{page.ctaPrimaryLabel}</Link><Link href={page.ctaSecondaryHref} className="bg-white/25 border border-navy-900/20 text-navy-900 font-display font-700 px-6 py-3 rounded hover:bg-white/40 transition-colors">{page.ctaSecondaryLabel}</Link></div></div></section>;
    return null;
  });
  return <>{rendered}</>;
}
