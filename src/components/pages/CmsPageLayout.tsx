...
export default function CmsPageLayout({ page, pageKey = "page", visualWidgets = {}, contact, children, formTitle }: { page: CmsPage; pageKey?: string; visualWidgets?: SiteContent["visualWidgets"]; contact?: Contact; children?: React.ReactNode; formTitle?: string }) {
  ...
  const rendered = order.map((section, index) => {
    if (section === "hero") return <><VisualWidgetZone pageKey={pageKey} zone={`before-${section}-${index}`} widgets={widgetsFor(`before-${section}-${index}`)} /><HeroSection key={`${section}-${index}`} page={page} /><VisualWidgetZone pageKey={pageKey} zone={`after-${section}-${index}`} widgets={widgetsFor(`after-${section}-${index}`)} /></>;
    if (section === "contactBar") return <><VisualWidgetZone pageKey={pageKey} zone={`before-${section}-${index}`} widgets={widgetsFor(`before-${section}-${index}`)} /><ContactBar key={`${section}-${index}`} contact={contact} /><VisualWidgetZone pageKey={pageKey} zone={`after-${section}-${index}`} widgets={widgetsFor(`after-${section}-${index}`)} /></>;
    if (section === "content") return <><VisualWidgetZone pageKey={pageKey} zone={`before-${section}-${index}`} widgets={widgetsFor(`before-${section}-${index}`)} /><ContentSection key={`${section}-${index}`} page={page} /><VisualWidgetZone pageKey={pageKey} zone={`after-${section}-${index}`} widgets={widgetsFor(`after-${section}-${index}`)} /></>;
    if (section === "process") return <><VisualWidgetZone pageKey={pageKey} zone={`before-${section}-${index}`} widgets={widgetsFor(`before-${section}-${index}`)} /><ProcessSection key={`${section}-${index}`} steps={page.steps} /><VisualWidgetZone pageKey={pageKey} zone={`after-${section}-${index}`} widgets={widgetsFor(`after-${section}-${index}`)} /></>;
    if (section === "formOrCta") return children ? <><VisualWidgetZone pageKey={pageKey} zone={`before-${section}-${index}`} widgets={widgetsFor(`before-${section}-${index}`)} /><section key={`${section}-${index}`} id="request" className="py-14 bg-white"><div className="max-w-3xl mx-auto px-4"><p className="section-label">Get Started</p><h2 className="section-heading text-3xl mb-2">{formTitle || page.ctaHeading}</h2><Paragraphs text={page.ctaBody} className="text-gray-500 mb-8 text-sm space-y-2" />{children}</div></section><VisualWidgetZone pageKey={pageKey} zone={`after-${section}-${index}`} widgets={widgetsFor(`after-${section}-${index}`)} /></> : <><VisualWidgetZone pageKey={pageKey} zone={`before-${section}-${index}`} widgets={widgetsFor(`before-${section}-${index}`)} /><section key={`${section}-${index}`} className="py-12 bg-accent"><div className="max-w-3xl mx-auto px-4 text-center"><h2 className="font-display font-900 text-3xl text-navy-900 mb-3">{page.ctaHeading}</h2><Paragraphs text={page.ctaBody} className="text-navy-800 mb-6 text-sm space-y-2" /><div className="flex flex-wrap justify-center gap-3">{page.ctaPrimaryLabel !== "__HIDDEN__" && page.ctaPrimaryLabel ? <Link href={page.ctaPrimaryHref} className="bg-navy-900 text-white font-display font-700 px-6 py-3 rounded hover:bg-navy-800 transition-colors">{page.ctaPrimaryLabel}</Link> : null}{page.ctaSecondaryLabel !== "__HIDDEN__" && page.ctaSecondaryLabel ? <Link href={page.ctaSecondaryHref} className="bg-white/25 border border-navy-900/20 text-navy-900 font-display font-700 px-6 py-3 rounded hover:bg-white/40 transition-colors">{page.ctaSecondaryLabel}</Link> : null}</div></div></section><VisualWidgetZone pageKey={pageKey} zone={`after-${section}-${index}`} widgets={widgetsFor(`after-${section}-${index}`)} /></>;
    return null;
  });
  return <>{rendered}</>;
}
