      ...
      <div className="max-w-7xl mx-auto px-4 py-8 grid lg:grid-cols-[1fr_360px] gap-6">
        <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
          {summary.lines.map(({ product, qty, lineTotal }) => {
            const unavailable = product.stockQty <= 0 || product.priceOnRequest || product.price === null || qty > product.stockQty;
            return (
              <div key={product.sku} className="p-5 border-b border-gray-100 last:border-b-0 grid md:grid-cols-[96px_1fr_auto] gap-4">
                ...
                <div>
                  <div className="flex flex-wrap items-center gap-2 mb-1">
                    <span className="font-mono text-[11px] text-gray-400">{product.sku}</span>
                    {unavailable && <span className="badge text-red-700 bg-red-50 border-red-200">Action needed</span>}
                  </div>
                  <Link href={`/shop/${product.slug}`} className="font-display font-800 text-navy-950 hover:text-accent transition-colors">{product.title}</Link>
                  <div className="grid sm:grid-cols-2 gap-x-6 gap-y-1 text-xs text-gray-500 mt-2">
                    ...
                    <p><span className="text-gray-400">Stock:</span> {product.stockQty}</p>
                  </div>
                  {unavailable && (
                ...
                <div className="md:text-right">
                  ...
                  <div className="inline-flex items-center border border-gray-200 rounded-lg overflow-hidden mb-3">
                    <button onClick={() => changeQty(product.sku, qty - 1)} className="p-2 hover:bg-gray-50" aria-label="Decrease quantity"><Minus size={13} /></button>
                    <span className="px-3 text-sm font-display font-700 min-w-10 text-center">{qty}</span>
                    <button onClick={() => changeQty(product.sku, qty + 1)} className="p-2 hover:bg-gray-50" aria-label="Increase quantity"><Plus size={13} /></button>
                  </div>
                  <button onClick={() => remove(product.sku)} className="flex md:ml-auto items-center gap-1 text-xs text-red-500 hover:text-red-700 font-600">
