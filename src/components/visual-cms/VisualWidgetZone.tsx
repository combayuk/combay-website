...
function widgetTextStyle(widget: VisualWidget): React.CSSProperties {
  return {
    fontSize: widget.fontSize || undefined,
    fontFamily: widget.fontFamily || undefined,
...
function buttonClass(style?: string) {
  if (style === "outline") return "inline-flex rounded-md border border-slate-300 bg-white px-5 py-2.5 text-sm font-900 text-[#2D4F7A] shadow-sm transition-colors hover:border-[#2D4F7A]";
  if (style === "secondary") return "btn-secondary";
  return "btn-primary";
}

function sectionBg(variant?: string) {
  if (variant === "dark") return "bg-[#2D4F7A] text-white border-[#2D4F7A]";
...
export function VisualWidgetBlock({ widget, index, zone, pageKey, allWidgets }: { widget: VisualWidget; index: number; zone: string; pageKey: string; allWidgets?: VisualWidgetsMap }) {
    ...
    return (
      <WidgetChrome widget={widget} index={index} zone={zone} className="lg:col-span-12">
        <section className={`${sectionBg(widget.sectionVariant)} rounded-2xl border px-5 py-8 shadow-sm`} style={style}>
          {(widget.title || widget.body) ? <div className="mx-auto mb-6 max-w-4xl text-center">{widget.title ? <h2 className="font-display text-2xl font-900 tracking-[-0.02em] text-[#2D4F7A]" style={widgetTextStyle(widget)}>{widget.title}</h2> : null}{widget.body ? <p className="mt-2 text-sm leading-7 text-slate-600">{widget.body}</p> : null}</div> : null}
  ...
  if (type === "button") return <WidgetChrome widget={widget} index={index} zone={zone} className={alignClass(widget.align)}><div className="py-2" style={style}><Link href={normalizeHref(widget.url)} target={widget.openInNewTab ? "_blank" : undefined} className={buttonClass(widget.buttonStyle)} style={widgetTextStyle(widget)}>{widget.text || widget.title || widget.linkLabel || "Button"}</Link></div></WidgetChrome>;
  if (type === "video") return <WidgetChrome widget={widget} index={index} zone={zone}><div className="overflow-hidden rounded-xl border border-slate-200 bg-[#2D4F7A] text-white shadow-sm" style={style}>{widget.videoUrl ? <video src={widget.videoUrl} poster={widget.thumbnailUrl || undefined} controls autoPlay={!!widget.autoplay} muted={!!widget.muted} loop={!!widget.loop} className="aspect-video w-full bg-black object-cover" /> : <div className="flex aspect-video items-center justify-center bg-[#253F62]"><div className="text-center"><div className="mx-auto mb-3 flex h-14 w-14 items-center justify-center rounded-full bg-white/10 text-2xl">▶</div><p className="font-display text-sm font-900">Video placeholder</p><p className="mt-1 text-xs text-white/60">Select this widget to add a video URL or upload a video.</p></div></div>}{(widget.title || widget.caption) ? <div className="p-4">{widget.title ? <h3 className="font-display font-900">{widget.title}</h3> : null}{widget.caption ? <p className="mt-1 text-sm text-white/65">{widget.caption}</p> : null}</div> : null}</div></WidgetChrome>;
  if (type === "promotion") return <WidgetChrome widget={widget} index={index} zone={zone} className="lg:col-span-12"><div className="my-4 rounded-xl border border-[#E6C06E] bg-[#FFF8E8] px-5 py-4 shadow-sm" style={style}><div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div><p className="font-mono text-[10px] font-800 uppercase tracking-widest text-[#C9872F]">Promotion</p><h3 className="font-display text-lg font-900 text-[#2D4F7A]" style={widgetTextStyle(widget)}>{widget.title || "Promotion banner"}</h3>{widget.body ? <p className="mt-1 text-sm text-slate-600">{widget.body}</p> : null}{widget.promoCode ? <p className="mt-2 inline-flex rounded bg-white px-2 py-1 font-mono text-xs font-900 text-[#2D4F7A] ring-1 ring-[#E6C06E]">Code: {widget.promoCode}</p> : null}</div>{widget.linkLabel ? <Link href={normalizeHref(widget.url)} className="btn-primary whitespace-nowrap">{widget.linkLabel}</Link> : null}</div></div></WidgetChrome>;
  if (type === "text") { const Tag = widget.textKind === "heading" ? "h2" : widget.textKind === "subheading" ? "h3" : "p"; const cls = widget.textKind === "heading" ? "font-display text-3xl font-900 text-[#2D4F7A]" : widget.textKind === "subheading" ? "font-display text-xl font-900 text-[#253F62]" : widget.textKind === "caption" ? "text-xs leading-7 text-slate-500" : "text-sm leading-7 text-slate-600"; return <WidgetChrome widget={widget} index={index} zone={zone} className={alignClass(widget.align)}><div className="py-2" style={style}><Tag className={cls} style={widgetTextStyle(widget)}>{widget.text || widget.title || "New text"}</Tag></div></WidgetChrome>; }
...
export default function VisualWidgetZone({ pageKey, zone, widgets, allWidgets, className = "", compact = false }: VisualWidgetZoneProps) {
  ...
  return (
    <section data-vcms-dropzone={fullZone} className={`group/vcms-zone relative ${className}`}>
      {visualCms ? <div className="pointer-events-none absolute inset-x-0 top-0 z-10 hidden -translate-y-1/2 items-center justify-center group-hover/vcms-zone:flex"><span className="rounded-full bg-[#E8A44A] px-3 py-1 text-[11px] font-900 text-[#2D4F7A] shadow">+ add section/widget here</span></div> : null}
