    ...
    return (
      <WidgetChrome widget={widget} index={index} zone={zone} className="lg:col-span-12">
        <section className={`${sectionBg(widget.sectionVariant)} rounded-2xl border px-5 py-8 shadow-sm`} style={style}>
          {(widget.title || widget.body) ? <div className="mx-auto mb-6 max-w-4xl text-center">{widget.title ? <h2 className="font-display text-2xl font-900 tracking-[-0.02em] text-[#2D4F7A]">{widget.title}</h2> : null}{widget.body ? <p className="mt-2 text-sm leading-7 text-slate-600">{widget.body}</p> : null}</div> : null}
